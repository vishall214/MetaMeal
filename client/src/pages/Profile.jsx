...
import { FaUser, FaWeight, FaExclamationTriangle } from 'react-icons/fa';
...
export default function Profile() {
  ...
  // Check if profile is complete for ML recommendations
  const isProfileComplete = () => {
    return !!(formData.profile.weight && formData.profile.height && formData.profile.age && formData.profile.gender);
  };

  // Update this check when formData changes
  useEffect(() => {
    setShowIncompleteProfileWarning(!isProfileComplete());
  }, [formData.profile.weight, formData.profile.height, formData.profile.age, formData.profile.gender]);
  ...
  useEffect(() => {
    const fetchUserProfile = async () => {
      try {
        ...
        // Structure the data properly - Handle both existing and quiz-populated data
        const profileData = {
          name: userData.name || '',
          email: userData.email || '',
          profile: {
            // Ensure numeric values are properly handled
            height: userData.profile?.height ? Number(userData.profile.height) : '',
            weight: userData.profile?.weight ? Number(userData.profile.weight) : '',
            age: userData.profile?.age ? Number(userData.profile.age) : '',
            gender: userData.profile?.gender || '',
            activityLevel: userData.profile?.activityLevel || '',
            // Handle arrays properly
            dietaryRestrictions: Array.isArray(userData.profile?.dietaryRestrictions) 
              ? userData.profile.dietaryRestrictions 
              : [],
            allergies: Array.isArray(userData.profile?.allergies) 
              ? userData.profile.allergies 
              : [],
            goals: Array.isArray(userData.profile?.goals) 
              ? userData.profile.goals 
              : [],
            healthConditions: Array.isArray(userData.profile?.healthConditions) 
              ? userData.profile.healthConditions 
              : [],
            // Handle quiz-specific fields
            filters: Array.isArray(userData.profile?.filters) 
              ? userData.profile.filters 
              : [],
            avatar: userData.profile?.avatar || ''
          },
          preferences: {
            mealTypes: Array.isArray(userData.preferences?.mealTypes) 
              ? userData.preferences.mealTypes 
              : [],
            cuisineTypes: Array.isArray(userData.preferences?.cuisineTypes) 
              ? userData.preferences.cuisineTypes 
              : [],
        ...
        // Check if profile is complete for ML recommendations after data is set
        setTimeout(() => {
          const isComplete = !!(profileData.profile.weight && profileData.profile.height && profileData.profile.age && profileData.profile.gender);
          setShowIncompleteProfileWarning(!isComplete);
    ...
    <PageContainer>
      ...
      {showIncompleteProfileWarning && (
        <WarningBanner>
          ...
          Complete your profile (weight, height, age, gender) to get personalized workout recommendations!
        </WarningBanner>
      )}
        ...
        <ProfileContent>
          <Form onSubmit={handleSubmit}>
            ...
            <Section>
              ...
              <FormGroup>
                <Label>Dietary Preferences (from Quiz)</Label>
                {isEditing ? (
                  <Input
                    type="text"
                    name="profile.filters"
                    value={formData.profile.filters.join(', ')}
                    onChange={handleArrayChange}
                    placeholder="e.g. veg, non-veg, diabetes, thyroid"
                  />
                ) : (
                  <DisplayValue>
                    {formData.profile.filters.length > 0 ? 
                      formData.profile.filters.join(', ') : 
                      'None specified'
                    }
                  </DisplayValue>
                )}

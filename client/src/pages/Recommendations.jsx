...
export default function Recommendations() {
  ...
  const generateRecommendations = async (showToast = true) => {
    try {
      ...
      const response = await api.post('/ml/predict-workout', requestData);
      ...
      let errorMessage = 'Failed to generate workout recommendations. Please try again.';
  ...
  useEffect(() => {
    // Auto-generate recommendations if user is logged in (without toast)
    if (user && user.id) {
        ...
        <ErrorContainer>
          ...
          {error.includes('Missing:') && (
            <p style={{ marginTop: '1rem', fontStyle: 'italic' }}>
              Please go to your Profile page and complete the missing information.
            </p>
          )}
